        ...
        keyOrder.forEach(key => {
            if (grouped[key].length > 1) {
                let sorted = [...grouped[key].sort(function (first, second) {
                    ...
                    if (Number(firstParts[0]) > Number(secondParts[0])) {
                        return -1;
                    } else if (Number(firstParts[0]) < Number(secondParts[0])) {
                        return 1
                    } else {
                        if (Number(firstParts[1]) > Number(secondParts[1])) {
                            return -1;
                        } else if (Number(firstParts[1]) < Number(secondParts[1])) {
                            return 1
                        }
                    }
