...
import likeService from './../../services/nistagram api/like.service';
import commentService from './../../services/nistagram api/comment.service';
import favoritesService from './../../services/nistagram api/favorites.service';
...
function Post (props) {
    ...
    useEffect(() => {
        ...
        changeLikesText();
        changeDislikesText()
        if(store.apiKey.jwt !== "" && store.apiKey.role!="Admin") getUserCollections()
    }, [])
    ...
    useEffect(()=>{
        ...
        setIsLiked(postLikes.filter(like => like.userId === store.apiKey.id).length === 1)
        setIsDisliked(postDislikes.filter(dislike => dislike.userId === store.apiKey.id).length === 1)
    }, [post])
    ...
    const getUserInfo = async () => {
        if(post.userId && post.userId !== store.apiKey.id){
            ...
            setUser({
                id: store.apiKey.id,
                username: store.apiKey.username,
                profilePhoto: store.apiKey.photo,
            })
        }
    }

    const handleLikeClick = async () => store.apiKey.jwt && await _handleLikeDislikeClick(true)
    const handleDislikeClick = async () => store.apiKey.jwt && await _handleLikeDislikeClick(false)
    ...
    const _handleLikeDislikeClick = async (isLike) => {
        ...
        if(response.status === 200){
            ...
            if(isLike) {
                ...
                if(isLiked){
                    changedPost.likes = changedPost.likes.filter(like => like.userId !== store.apiKey.id)
                }else{
                    changedPost.likes.push(newItem)
                    // Remove existing dislike
                    if(isDisliked) changedPost.dislikes = changedPost.dislikes.filter(dislike => dislike.userId !== store.apiKey.id)
                }
            }else{
                if(isDisliked) {
                    changedPost.dislikes = changedPost.dislikes.filter(dislike => dislike.userId !== store.apiKey.id)
                }else{
                    changedPost.dislikes.push(newItem)
                    // Remove existing like
                    if(isLiked) changedPost.likes = changedPost.likes.filter(like => like.userId !== store.apiKey.id)
                }
            }
            setPost(changedPost);
        }else{
    ...
    const handleSaveClick = () => {
        store.apiKey.jwt && setShowSaveModal(!showSaveModal);
    }
    ...
    return(
        <div className="Post">
            ...
            <CollectionsModal 
                showModal={store.apiKey.jwt !== "" && showSaveModal} 
